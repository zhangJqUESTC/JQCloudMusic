...
#pragma mark 请求列表
+ (void)requestListObjectWith:(Class)clazz url:(NSString *)url parameters:(nullable NSDictionary *)parameters cachePolicy:(MSCachePolicy)cachePolicy loading:(BOOL)loading controller:(nullable BaseLogicController *)controller success:(SuperHttpListSuccess)success failure:(SuperHttpFail)failure{
    
    //前置处理
    [self preProcess:controller];
    
    [self requestWithMethod:MSRequestMethodGET url:url parameters:parameters cachePolicy:cachePolicy success:^(NSURLSessionDataTask *task,id  _Nonnull data) {
